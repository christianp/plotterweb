import {group, stroke, attr, choice,polyline, path,fill,rect,line,circle,uniform,element} from '/static/page.js';
...
function snowman(x,y,r) {
    y -= r;
    const g = group();
    g.appendChild(fill(circle(x,y,r)));
    g.appendChild(fill(circle(x,y-r,r*0.5)));
    const a = uniform(-PI/4,0);
    const arm_start = 0.8;
    const arm_length = r;
    const [x1,y1] = [cos(PI/4)*r*arm_start, -sin(PI/4)*r*arm_start];
    const [x2,y2] = [x1 + cos(a)*arm_length, y1+sin(a)*arm_length];
    g.appendChild(line(x+x1,y+y1,x+x2,y+y2));
    g.appendChild(line(x-x1,y+y1,x-x2,y+y2));
    return g;
}
...
const n = 4;
for(let i=0;i<n;i++) {
    const r = uniform(5,15);
    stroke(snowman(
        (i+1)/(n+1)*W,
        H-20,///uniform(H-r-20,H-r),
        r
    ),['blue','black','green','red'][i]);
}

/*
for(let i=0;i<5;i++) {
    const [w,h] = [uniform(5,20), uniform(5,20)];
    const [x,y] = [uniform(0,30-w), uniform(0,30-h)];
    fill(rect(x,y,w,h));
}
*/
